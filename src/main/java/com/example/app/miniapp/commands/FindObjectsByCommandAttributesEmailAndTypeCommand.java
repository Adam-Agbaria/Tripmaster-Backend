...
@Component("findObjectsByCommandAttributesEmailAndType")
public class FindObjectsByCommandAttributesEmailAndTypeCommand implements MiniAppCommand {

    private final EntityObjectRepository entityObjectRepository;

    @Autowired
    public FindObjectsByCommandAttributesEmailAndTypeCommand(EntityObjectRepository entityObjectRepository) {
    ...
    public BoundaryCommand execute(BoundaryCommand boundaryCommand) {
        ...
        try {
            objectsPage = entityObjectRepository.findByTypeAndObjectDetailsEmail(email, type, PageRequest.of(page, size));
        } catch (Exception e) {
